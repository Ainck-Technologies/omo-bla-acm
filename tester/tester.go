...
func (this *Notification) Handle(_ctx context.Context, _message *pn.SimpleMessage) error {
	...
	if ok {
		fmt.Println(fmt.Sprintf("[omo.bla.acm.notification] Received message %+v with metadata %+v", _message, md))
	} else {
		fmt.Println(fmt.Sprintf("[omo.bla.acm.notification] Received message %+v without metadata", _message))
	}
	return nil
}
...
func main() {
	...
	service := micro.NewService(
		micro.Name("omo.bla.acm.tester"),
	)
	service.Init()

	micro.RegisterSubscriber("omo.bla.acm.notification", service.Server(), new(Notification))
	...
	auth := proto.NewAuthService("omo.bla.acm", cli)
	profile := proto.NewProfileService("omo.bla.acm", cli)
